...
layout.forEach((hexCount, rowIndex) => {
  ...
  for (let i = 0; i < hexCount; i++) {
    ...
    if (isEdge) {
      edgeHexes.push({ hex, x, y, rowIndex, colIndex: i });
    } else {
...
// Agrupar edgeHexes por fila
const edgeHexesByRow = new Map();
for (const edge of edgeHexes) {
  const y = Math.round(edge.y);
  if (!edgeHexesByRow.has(y)) {
    edgeHexesByRow.set(y, []);
  }
  edgeHexesByRow.get(y).push(edge);
}

// Ordenar filas de arriba a abajo
const sortedRows = Array.from(edgeHexesByRow.entries()).sort((a, b) => a[0] - b[0]);
let portIndex = 0;

const puertoRotations = {
  0: [0, 60],
  1: [60],
  2: [300],
  3: [120],
...
sortedRows.forEach(([y, row], rowIdx) => {
  row.sort((a, b) => a.x - b.x);

  let portInRow = 0;

  row.forEach((edge, i) => {
    const shouldPlacePort = rowIdx % 2 === 0
      ? i % 2 === 0
      : i % 2 !== 0;

    if (shouldPlacePort && portIndex < portTypes.length) {
      const tipo = portTypes[portIndex++];

      // Obtener rotación definida, si existe
      const rotation = puertoRotations[rowIdx]?.[portInRow] ?? 0;

      edge.hex.style.backgroundImage = `url(${portImages[tipo]})`;
      edge.hex.style.transform = `rotate(${rotation}deg)`;

      portInRow++;
    } else {
      edge.hex.style.backgroundImage = `url(${portImages.agua})`;
      edge.hex.style.transform = 'rotate(0deg)';
    }
  });
});
...
function roundToGrid(value, gridSize = 36) {
