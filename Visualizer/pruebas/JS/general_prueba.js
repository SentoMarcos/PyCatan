...
function clearLogs() {
    jQuery('#commerce_log_text').empty();
    jQuery('#other_useful_info_text').empty();
}

function addCommerceLog(message) {
    const commerceLog = jQuery('#commerce_log_text');
    commerceLog.append('<div class="log-entry">' + message + '</div>');
    // Auto-scroll to bottom
    commerceLog.scrollTop(commerceLog[0].scrollHeight);
}

function addOtherInfoLog(message) {
    const otherInfoLog = jQuery('#other_useful_info_text');
    otherInfoLog.append('<div class="log-entry">' + message + '</div>');
    // Auto-scroll to bottom
    otherInfoLog.scrollTop(otherInfoLog[0].scrollHeight);
}

function updateLogFromGameState() {
    // This function will be called when game state changes
    // to automatically update logs based on current phase
    if (typeof contador_fases !== 'undefined' && contador_fases.val()) {
        // Force a refresh of the current phase to update logs
        contador_fases.trigger('change');
    }
}
...
// Funciones para manejar los logs
function clearLogs() {
...
function addCommerceLog(message) {
    const logElement = $('#commerce_log_text');
    logElement.append('<div class="log-entry">' + message + '</div>');
    logElement.scrollTop(logElement[0].scrollHeight);
}

function addOtherInfoLog(message) {
    const logElement = $('#other_useful_info_text');
    logElement.append('<div class="log-entry">' + message + '</div>');
    logElement.scrollTop(logElement[0].scrollHeight);
}
...
function addSetupBuildings() {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        for (let j = 0; j < setup_obj['P' + i].length; j++) {
            ...
            textarea.text(textarea.text() + str);            paint_it_player_color(i, node);
            paint_it_player_color(i, road);

            node.html('<i class="fa-solid fa-house center-house"></i>');
        }
    }
}
...
function init_events_with_game_obj() {
    ...
    contador_fases.off().on('change', function (e) {
        ...
        // Actualizar el log con información del estado actual del juego
        updateLogFromGameState();
        switch (parseInt(contador_fases.val()) - 1) {
            case 0:
                ...
                break;
            case 1:                phase_obj = turn_obj['commerce_phase'];
                clearLogs(); // Limpiar logs usando la nueva función
                ...
                for (let i = 0; i < phase_obj.length; i++) {
                    ...
                    if (phase_obj[i]['trade_offer'] == 'None') {
                        ...
                        if (phase_obj.length == 1) {
                            const noTradeMsg = `P${contador_turnos.val()}: No trade`;
                            addCommerceLog(noTradeMsg);
                        }

                        break;
                    }
                    if (phase_obj[i]['inviable']) {
                        const inviableMsg = `P${contador_turnos.val()}: Inviable trade`;
                        addCommerceLog(inviableMsg);
                    ...
                    if (phase_obj[i]['harbor_trade']) {
                        ...
                        const harborTradeMsg = `P${contador_turnos.val()}: Harbor trade - Gives: ${material_given.charAt(0).toUpperCase() + material_given.slice(1)}, Receives: ${material_received.charAt(0).toUpperCase() + material_received.slice(1)}`;
                        addCommerceLog(harborTradeMsg);
                        ...
                        const cardPlayedMsg = `P${contador_turnos.val()}: Played a development card`;
                        addCommerceLog(cardPlayedMsg);

                        if (game_direction === 'forward') {
                            on_development_card_played(phase_obj[i]['development_card_played']);
                        }                    } else {
                        const receivesMsg = ` Receives: Cereal: ${phase_obj[i]['trade_offer']['receives']['cereal']}, Mineral: ${phase_obj[i]['trade_offer']['receives']['mineral']}, Wool: ${phase_obj[i]['trade_offer']['receives']['wool']}, Wood: ${phase_obj[i]['trade_offer']['receives']['wood']}, Clay: ${phase_obj[i]['trade_offer']['receives']['clay']}`;
                        const offerMsg = `P${contador_turnos.val()}: Offer - Gives: Cereal: ${phase_obj[i]['trade_offer']['gives']['cereal']}, Mineral: ${phase_obj[i]['trade_offer']['gives']['mineral']}, Wool: ${phase_obj[i]['trade_offer']['gives']['wool']}, Wood: ${phase_obj[i]['trade_offer']['gives']['wood']}, Clay: ${phase_obj[i]['trade_offer']['gives']['clay']}${receivesMsg}`;
                        addCommerceLog(offerMsg);                        for (let j = 0; j < phase_obj[i]['answers'].length; j++) {
                            ...
                            for (let n = 0; n < phase_obj[i]['answers'][j].length; n++) {
                                ...
                                if (phase_obj[i]['answers'][j][n]['count'] == 1) {
                                    ...
                                    if (phase_obj[i]['answers'][j][n]['response'] == true) {
                                        const playerReceiver = parseInt(phase_obj[i]['answers'][j][n]['receiver']) + 1;
                                        if (phase_obj[i]['answers'][j][n]['completed']) {
                                            const acceptedMsg = `P${playerReceiver}: Accepted`;
                                            addCommerceLog(acceptedMsg);
                                            ...
                                            // fin añadir materiales a mano                                        } else {
                                            const cannotCompleteMsg = `P${playerReceiver}: Accepted - Cannot be completed (lack of materials)`;
                                            addCommerceLog(cannotCompleteMsg);
                                        }
                                    } else {
                                        if (phase_obj[i]['answers'][j][n]['count'] == phase_obj[i]['answers'][j].length) {
                                            const playerReceiver = parseInt(phase_obj[i]['answers'][j][n]['receiver']) + 1;
                                            const deniedMsg = `P${playerReceiver}: Denied`;
                                            addCommerceLog(deniedMsg);
                                        }
                                    }
                                } else {
                                    counteroffer_counter++;
                                    // hay contraoferta
                                    const giver = phase_obj[i]['answers'][j][n]['giver'] + 1;
                                    const gives = phase_obj[i]['answers'][j][n]['trade_offer']['gives'];
                                    const receives = phase_obj[i]['answers'][j][n]['trade_offer']['receives'];
                                    const counterofferMsg = `P${giver}: Counteroffer - Gives: Cereal: ${gives['cereal']}, Mineral: ${gives['mineral']}, Wool: ${gives['wool']}, Wood: ${gives['wood']}, Clay: ${gives['clay']} - Receives: Cereal: ${receives['cereal']}, Mineral: ${receives['mineral']}, Wool: ${receives['wool']}, Wood: ${receives['wood']}, Clay: ${receives['clay']}`;
                                    addCommerceLog(counterofferMsg);
                                    ...
                                    if (phase_obj[i]['answers'][j][n]['response'] == true) {
                                        ...
                                        if (phase_obj[i]['answers'][j][n]['count'] == phase_obj[i]['answers'][j].length) {
                                            ...
                                            // se niega la oferta pero se ofrece contraoferta
                                            const counterOfferMsg = `P${playerReceiver}: Counter-offer`;
                                            addCommerceLog(counterOfferMsg);
                                            counteroffer_counter++;
                                        }
                                    }                                }                            }
                ...
                // Commerce phase logging completed using addCommerceLog() functions
                        ...
                        if (phase_obj[i]['building'] !== null) {
                            switch (phase_obj[i]['building']) {
                                case 'town':
                                    ...
                                    break;
                                case 'city':                                    if (phase_obj[i]['finished']) {
                                        let node = jQuery('#node_' + phase_obj[i]['node_id']);
                                        node.html('<i class="fa-solid fa-house center-house"></i>');
                        ...
                        switch (phase_obj[i]['building']) {
                            case 'town':
                                if (phase_obj[i]['finished']) {
                                    ...
                                    const townMsg = `P${contador_turnos.val()}: Built Town at Node ${phase_obj[i]['node_id']}`;
                                    addOtherInfoLog(townMsg);
                                }
                                break;
                            case 'city':
                                if (phase_obj[i]['finished']) {
                                    let node = jQuery('#node_' + phase_obj[i]['node_id']);
                                    node.html('<i class="fa-solid fa-chess-rook"></i>');                                    if (game_direction === 'forward') {
                                        changeHandObject(actual_player_json, {
                                            'cereal': cereal_quantity_text - 2,
                                            'mineral': mineral_quantity_text - 3,
                                            'clay': clay_quantity_text - 0,
                                    ...
                                    const cityMsg = `P${contador_turnos.val()}: Built City at Node ${phase_obj[i]['node_id']}`;
                                    addOtherInfoLog(cityMsg);
                                }
                                break;
                            case 'road':
                                if (phase_obj[i]['finished']) {
                                    ...
                                    const roadMsg = `P${contador_turnos.val()}: Built Road from Node ${phase_obj[i]['node_id']} to Node ${phase_obj[i]['road_to']}`;
                                    addOtherInfoLog(roadMsg);
                                }
                                break;
                            case 'card':
                                ...
                                if (phase_obj[i]['finished']) {
                                    ...
                                    card_div.addClass('increased');                                    if (game_direction === 'forward') {
                                        changeHandObject(actual_player_json, {
                                            'cereal': cereal_quantity_text - 1,
                                            'mineral': mineral_quantity_text - 1,
                                            'clay': clay_quantity_text - 0,
                                    ...
                                    const cardMsg = `P${contador_turnos.val()}: Bought Development Card - ${card_effects[phase_obj[i]['card_effect']]}`;
                                    addOtherInfoLog(cardMsg);
                                }
                                break;
                            case 'played_card':
                                if (phase_obj[i]['development_card_played'] && game_direction === 'forward') {
                                    on_development_card_played(phase_obj[i]['development_card_played'])
                                    const playedCardMsg = `P${contador_turnos.val()}: Played Development Card`;
                                    addOtherInfoLog(playedCardMsg);
                                }
                                break;
                            default:
                                break;
                ...
                if (phase_obj['development_card_played'] && phase_obj['development_card_played'].length && game_direction === 'forward') {
                    ...
                    // console.log(phase_obj)
                    on_development_card_played(phase_obj['development_card_played'][0])
                    const endTurnCardMsg = `P${contador_turnos.val()}: Played Development Card at end of turn`;
                    addOtherInfoLog(endTurnCardMsg);
                }
                ...
                if (winner !== '' && contador_turnos.val() >= 4) {
                    const winnerMsg = `Game Over: ${winner}`;
                    addOtherInfoLog(winnerMsg);
                    alert(winner);
                }

                const endTurnMsg = `P${contador_turnos.val()}: Turn ended - Victory Points: ${phase_obj['victory_points']['J' + (actual_player_json)]}`;
                addOtherInfoLog(endTurnMsg);
